# Contextual districting through a capacitated-MST decoder, the problem class
# of DistrictNet (Ahmed, Forel, Parmentier & Vidal, NeurIPS 2024,
# arXiv 2412.08287), solved zeroth-order through a hard, non-differentiable
# decoder. This is a self-contained re-creation of the problem
# class, not DistrictNet's architecture (their CMST layer is smoothed for
# gradient training); the Esau-Williams decoder carries its known ~5-10% CMST
# optimality gap.
#
# Pipeline: edge features -> tiny MLP (D=57 flat params) -> predicted edge
# costs -> Esau-Williams capacitated MST -> districts -> loss = sum of
# district TSP tours on the true costs. The true costs include a "river"
# multiplier the raw distances don't reveal, so the encoder has something to
# learn; decoding with the true costs gives a strong reference.
#
# Eval accounting rule (fair budgets): the unit is one decoder call
# (EW + TSP). Perturbed-gradient methods pay M decoder calls per gradient.
#
...
function make_instance(; n = NCUST, seed = 1)
    ...
    for i in 1:m, j in 1:m
        ...
        tc[i, j] = len * (1 + 2 * cr)          # the river multiplier is latent
...
function predict_costs!(pc::Matrix{Float64}, θ::AbstractVector, inst::Instance)
    m = size(pc, 1)
    W1 = reshape(view(θ, 1:40), 8, 5)
    b1 = view(θ, 41:48)
    w2 = view(θ, 49:56)
    b2 = θ[57]
    h = zeros(8)
...
# ---------------------------------------------------------------------------
# Esau-Williams capacitated MST (1966): start from a star out of the depot,
# repeatedly apply the best positive saving s_i = gate(i) - cheapest feasible
# link, respecting the per-subtree capacity. O(n^2) scans per merge,
# fine at n = 60; priority queue if this ever grows.
# ---------------------------------------------------------------------------
...
# the full pipeline objective (one "decoder call" per θ)
function pipeline_loss(θ::AbstractVector, inst::Instance, pc::Matrix{Float64})
    predict_costs!(pc, θ, inst)
    return decode_cost(pc, inst)
end

# ---------------------------------------------------------------------------
# Baseline: perturbed-optimizer gradient (Berthet et al. 2020 style) + Adam,
# hand-rolled to keep eval accounting transparent: M decoder calls per grad.
# ---------------------------------------------------------------------------
function perturbed_adam(inst, θ0; sigma = 0.3, M = 16, lr = 0.05, budget = BUDGET, rng = Xoshiro(0))
    θ = copy(θ0)
    mt = zero(θ)
    vt = zero(θ)
    ...
    while used + M <= budget
        t += 1
        Z = randn(rng, length(θ), M)
        losses = [pipeline_loss(θ .+ sigma .* Z[:, k], inst, pc) for k in 1:M]
        used += M
        lmin = minimum(losses)
        lmin < best[1] && (best = (lmin, θ .+ sigma .* Z[:, argmin(losses)]))
        adv = (losses .- mean(losses)) ./ (std(losses) + 1e-8)
        g = (Z * adv) ./ (M * sigma)
        mt .= 0.9 .* mt .+ 0.1 .* g
        vt .= 0.999 .* vt .+ 0.001 .* g .^ 2
        θ .-= lr .* (mt ./ (1 - 0.9^t)) ./ (sqrt.(vt ./ (1 - 0.999^t)) .+ 1e-8)
    end
    return best
end
...
function main()
    ...
    # exact small-instance sanity check of the decoder
    ...
    for seed in seeds
        θ0 = 0.3 .* randn(Xoshiro(10 + seed), D_PARAM)
        pcl = zeros(NCUST + 1, NCUST + 1)
        f_b(X) = [pipeline_loss(view(X, :, j), inst, pcl) for j in 1:size(X, 2)]

        es = minimize(f_b, D_PARAM; steps = BUDGET ÷ (2 * D_PARAM), epsilon = 0.02,
            step_radius = 0.6, x0 = θ0, rng = Xoshiro(seed), scale_cost = :mean)
        push!(get!(results, "PolyStep", Float64[]), es.best_f)

        res = cma_minimize(x -> pipeline_loss(x, inst, pcl), copy(θ0), 0.3;
            maxfevals = BUDGET, verbosity = 0, seed = UInt(seed + 1))
        push!(get!(results, "CMA-ES", Float64[]), fbest(res))

        bf, _ = perturbed_adam(inst, θ0; rng = Xoshiro(seed))
        push!(get!(results, "Perturbed+Adam", Float64[]), bf)
    end

    @printf("  %-16s %14s %18s\n", "method", "cost (mean±sd)", "gap closed (mean)")
    println("  " * "-"^52)
    for name in ("PolyStep", "CMA-ES", "Perturbed+Adam")
        cs = results[name]
        @printf("  %-16s %8.3f ± %.3f %16.1f%%\n", name, mean(cs), std(cs), mean(gap.(cs)))
    end
    ps_gap = mean(gap.(results["PolyStep"]))
    println("  (gap closed = progress from random-theta toward true-cost decoding)")
    @assert ps_gap > 0 "PolyStep failed to improve over random initialization"
    return results
end
