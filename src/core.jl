...
[`columnwise`](@ref). Costs are measured at `probe_radius`, the step is taken
at `step_radius` (decoupled). `epsilon`/`step_radius`/`probe_radius` accept a
Real or an epsilon schedule; unscheduled radii are multiplied by the current
epsilon (scheduled ones are not).
...
Base.@kwdef struct PolyStepConfig{S <: AbstractOTSolver, E, EE, SR, PR, SC, LB, UB, RP}
    ...
    lb::LB = nothing                    # box bounds: probe points are clamped
    ub::UB = nothing                    # before eval and the iterate after stepping
...
function _validate(ps::PolyStepConfig)
    ...
    if ps.newton_refinement || ps.trust_region
        # Python no-ops these without use_quadratic_model; we error instead
        # (a documented difference from the reference).
    ...
    # radii have no state-copy or update! feedback path (only the OT epsilon
    # schedules do), so a ProgressiveEpsilon radius would freeze at init
    ...
    # constant epsilon/radii must have the right sign (schedules validate their
    # own outputs; negative radii would move against the transport direction and
    # eps <= 0 makes softmax(-C/eps) degenerate)
...
# ::F type parameters force closure specialization (an untyped captured `f`
# would dynamic-dispatch on every column)
...
mutable struct PolyStepState{T <: AbstractFloat, S <: AbstractOTSolver}
    ...
    best_x::Vector{T}                   # incumbent: best evaluated candidate
    best_f::Float64
    evals::Int
    # diagnostics
    ...
    Vt::Matrix{T}                       # (d, V) template
    scales::Vector{T}                   # (K,)
    Zc::Array{T, 3}                      # (d, d, P) Gaussian scratch
    R::Array{T, 3}                       # (d, d, P)
    ...
    # state-local copies of mutable schedules: keeps the shared PolyStep config
    # immutable in effect, so repeat/concurrent runs are independent
    ...
    # state-local solver copy: isolates the mutable workspace and warn latch so
    # repeat/concurrent runs from one shared PolyStep config stay independent
    solver::S
    # set when the last step had no finite candidate in the whole batch: the
    # divergence signal, since sanitize_cost! erases non-finite costs from Craw
...
function init_state(ps::PolyStepConfig, X0::AbstractMatrix{T}) where {T <: AbstractFloat}
    ...
    return PolyStepState{T, typeof(solver)}(
        ...
        polytope_vertices(T, ps.polytope, d),
        probe_scales(T, K),
        zeros(T, d, d, P), zeros(T, d, d, P),
        ps.polytope === :orthoplex ? nothing : zeros(T, d, V, P),
        zeros(T, d, K * V * P), losses,
        # losses3 aliases losses (reshape shares memory): the (K, V, P) view is
        # always current, no per-step copy
        ...
        ps.newton_refinement ? zeros(T, d, P) : nothing,    # N (refinement scratch)
...
# orthoplex fast path: rotated vertex (v=i, v=d+i) = +/-R[:, i, p]; probe
# points written directly from columns of R, no matmul, no (d,V,P) intermediate
...
# clamp probe columns into [lb, ub], flagging any particle whose probes moved; the
# FD model is invalid for clamped probes, so those columns are zeroed downstream
...
# bounds validity: finite and lb <= ub elementwise (scalar or (d,) vector, or a
# scalar/vector mix). Shape/pairing is checked separately by the caller.
_finite_bound(b::Real) = isfinite(b)
_finite_bound(b::AbstractVector) = all(isfinite, b)
function _check_bounds(lb, ub)
    _finite_bound(lb) || throw(ArgumentError("lb must be finite, got $lb"))
    _finite_bound(ub) || throw(ArgumentError("ub must be finite, got $ub"))
    all(lb .<= ub) ||
        throw(ArgumentError("require lb <= ub elementwise, got lb=$lb, ub=$ub"))
    return nothing
end

# mean over the contiguous K-run per (v, p); K=1 fast path is the same loop
...
proxy). `f` is the batched objective `(d, N) -> (N,)`.
"""
function step!(f, ps::PolyStepConfig, st::PolyStepState{T};
    ...
    # rotations (CPU, serial randn -> thread-count independent)
    haar_rotations!(st.R, st.Zc, rng)
    if ps.biased_rotation && st.prev_descent !== nothing && all(isfinite, st.prev_descent)
    ...
    # probe points (evaluated at probe radius; step happens at step radius)
    ...
    # bounds / repair apply to the evaluated candidates (lb/ub scalar or (d,))
    if ps.lb !== nothing
    ...
    # objective (dominant cost; chunked for memory when eval_chunk > 0)
    ncols = K * V * P
    if ps.eval_chunk > 0
        j = 1
        while j <= ncols
            hi = min(j + ps.eval_chunk - 1, ncols)
            st.losses[j:hi] .= f(view(st.Xprobe, :, j:hi))
            j = hi + 1
        end
    else
        st.losses .= f(st.Xprobe)
    end
    st.evals += ncols

    # incumbent: best evaluated candidate (feasible whenever bounds/repair
    # active); NaN-safe scan: findmin would return (NaN, i) if any loss is
    # NaN, discarding the batch's finite candidates
    ...
    # st.losses3 aliases st.losses in (K, V, P) shape; raw per-probe losses
    # for the quadratic model are already in place, no copy needed

    # Craw: sanitized unscaled (trust region / adaptive radius / diagnostics);
    # Csolve: scaled solver input; scale_cost must never leak into the former
    _cost_from_losses!(st.Craw, st.losses, K)
    sanitize_cost!(st.Craw)
    scale_cost!(st.Csolve, st.Craw, ps.scale_cost)

    # trust-region update: last step's predicted improvement vs this step's realized
    # change. The proxy is the mean over all probes, the same mean-over-particles
    # quantity predicted_improvement_mean estimates. Reused as prev_pre_step_loss below.
    ...
    # OT solve (Csolve is already sanitized+scaled; pass scale_cost=nothing);
    # warm-started duals are rescaled by eps_new/eps_old whenever the OT
    # epsilon moves, and non-finite duals reset to zero (see sinkhorn.jl)
    ...
    if st.solver isa SoftmaxSolver
        # fast path: softmax weights are already the mass-normalized plan, so skip
        # the public solve's copy/sanitize round-trip (still guard eps > 0)
        _check_eps(ot_eps)
        _warn_tiny_eps!(st.solver, ot_eps, st.Csolve)
        ...
        # feedback goes to the state-local schedule copy (shared config stays inert)
        ...
        # single projection path: normalize by realized column mass, then the
        # vertex-free centroid step (identical to explicit barycentric projection)
    ...
    if (ps.biased_rotation || ps.trust_region) && quad_ready
        ...
        fd_ready = true
        if st.clampflag !== nothing
            # clamped probes break the nominal-offset FD assumption: neutralize
            # that particle's model (zero gradient/curvature)
            @inbounds for p in 1:P
                if st.clampflag[p]
                    for i in 1:d
                        st.G[i, p] = zero(T)
        ...
        if ps.trust_region
            # predict the improvement of the taken OT step (sr*cent in the rotated
            # frame where G/H live), not the untaken Newton step, so the
            # pred-vs-actual ratio compares the same move
            st.prev_predicted = predicted_improvement_mean(st.G, st.H, st.cent; scale = sr)
            st.prev_pre_step_loss = tr_proxy
        end
    elseif ps.biased_rotation
        # OT fallback: weighted vertex offset = sr * delta (scale kept for parity)
        st.prev_descent === nothing && (st.prev_descent = zeros(T, d, P))
        @. st.prev_descent = sr * st.delta
    end

    # Newton refinement (post-OT move -> the solve's duals become stale). Reuses
    # st.G/st.H when the block above already computed them (fd_ready), and writes
    # into preallocated scratch (st.Xref/st.N/st.refrot), no per-step allocation.
    ...
    # bounds: project the iterate back into the box
    ...
    # revert on a NaN iterate, and also hold on a fully non-finite batch: the
    # uniform-penalty softmax only cancels to a no-op for symmetric unbounded
    # probes, so clamped/repaired candidates would otherwise drift the particle
    ...
    push!(st.costs, cost_mean)      # raw sanitized model cost (Python monolithic history)
...
function _converged(ps::PolyStepConfig, st::PolyStepState)
    ...
    d = st.disp_sqnorms
    # plateau: the per-step displacement has stopped changing
    plateau = abs(d[end] - d[end - 1]) / (abs(d[end - 1]) + 1e-10) < ps.threshold
    # magnitude check: convergence also needs a small step, not just a steady one
    # (a constant-velocity descent plateaus mid-run). Reference: peak displacement.
...
displacement-plateau convergence / divergence criteria starting after step
`min_iterations + 1` (the `i - 1 >= min_iterations` check mirrors the Python
reference's 0-based `i >= min_iterations`). `callback(st) -> true` stops early.
"""
